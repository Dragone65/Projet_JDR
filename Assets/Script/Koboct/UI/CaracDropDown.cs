...
namespace Koboct.UI
{
    public class CaracDropDown : MonoBehaviour
    {
        ...
        public TypeCharacteristique MyCarac;
        [SerializeField] private Characteristique characteristique;
        public UnityEvent CharacteristiqueChoiceFinished = new();

        private void Start()
        {
            if (_caracDropdown == null)
                _caracDropdown = GetComponent<TMP_Dropdown>();
            remainingValues = Service.MonResultatJetCharacteristique.ToList();
            ...
            characteristique = Service.GetCharacteristique(MyCarac);
            characteristique.OnValeurChange.AddListener(CaracChange);
        }

        private void CaracChange(int arg0)
        {
            var modificateur = characteristique.Modificateur;
            _modText.text = modificateur >= 0 ? $"+{modificateur}" : modificateur.ToString();
            _caracDropdown.captionText.text = characteristique.Valeur.ToString();
        }
        ...
        private void OnValueSelected(int index)
        {
            ...
            Service.SetCharacteristique(MyCarac, selectedValue);
...
#endif
                CharacteristiqueChoiceFinished.Invoke();
            }
        }
    }
}